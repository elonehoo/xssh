...
pub(crate) struct AppThemeId(&'static str);
...
impl AppThemeId {
    ...
    pub(crate) fn terminal_palette(self) -> TerminalThemePalette {
        app_theme_by_id(self).terminal_palette()
    }
    ...
    pub(crate) fn from_setting_value(value: &str) -> Self {
        APP_THEMES
            ...
            .map(|theme| AppThemeId::new(theme.id))
            .unwrap_or(DEFAULT_APP_THEME_ID)
    }
}

pub(crate) const DEFAULT_APP_THEME_ID: AppThemeId = AppThemeId::new("default-dark");

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum AppThemeKind {
    Dark,
    Light,
}
...
impl AppTheme {
    pub(super) fn terminal_palette(self) -> TerminalThemePalette {
...
#[derive(Clone)]
pub(crate) struct AppThemeChoice {
    id: AppThemeId,
    title: SharedString,
}
...
impl AppThemeChoice {
    fn new(theme: &AppTheme) -> Self {
        Self {
            id: AppThemeId::new(theme.id),
            title: theme.name.into(),
        }
    }
}

impl SelectItem for AppThemeChoice {
    type Value = AppThemeId;
...
pub(crate) fn app_theme_options() -> Vec<AppThemeChoice> {
    APP_THEMES.iter().map(AppThemeChoice::new).collect()
}

pub(crate) fn app_theme_selected_index(id: AppThemeId) -> IndexPath {
    let row = APP_THEMES
        .iter()
        .position(|theme| theme.id == id.0)
        .unwrap_or_else(default_app_theme_index);

    IndexPath::default().row(row)
}

pub(crate) fn app_theme_by_id(id: AppThemeId) -> &'static AppTheme {
    APP_THEMES
        .iter()
        .find(|theme| theme.id == id.0)
        .unwrap_or_else(default_app_theme)
}

fn default_app_theme_index() -> usize {
    APP_THEMES
        .iter()
        .position(|theme| theme.id == DEFAULT_APP_THEME_ID.0)
        .unwrap_or(0)
}

fn default_app_theme() -> &'static AppTheme {
    APP_THEMES
        .iter()
        .find(|theme| theme.id == DEFAULT_APP_THEME_ID.0)
        .unwrap_or(&APP_THEMES[0])
}

pub(crate) const APP_THEMES: [AppTheme; 94] = [
    AppTheme {
        id: "aquarium",
        name: "Aquarium",
